...
if (!class_exists('DividendModel')) {
	class DividendModel extends PluginModel
	{
		...
		public function checkOrderConfirm($orderid = '0')
		{
			...
			//默认分红
            $divedend['dividend_ratio_price'] = $divedend['dividend_price'];
			//经理
			$divedend['dividend_manager_price'] = round($order['price'] * ($set['manager'] / 100), 2);
            //合伙人
            $divedend['dividend_partner_price'] = round($order['price'] * ($set['partner'] / 100), 2);

			//覆盖
            $heads_member = m('member')->getMember($member['headsid']);
            switch ($heads_member['level']){
                case '2':
                    $divedend['dividend_price'] += $divedend['dividend_manager_price'];
                    break;
                case '3':
                    $divedend['dividend_price'] += $divedend['dividend_partner_price'];
                    break;
                default:
                    $divedend['dividend_price'] = $divedend['dividend_price'];
                    break;
            }
