...
class Posts_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'select p.id,p.title,p.createtime,p.content,p.images ,p.replytime, p.openid, p.nickname,p.avatar,p.isbest,p.isboardbest,p.istop,p.isboardtop,p.is_home_recom,
    ...
    public function homeRecom()
    {
        ...
        $home_recom = intval($_GPC['home_recom']);
        $items = pdo_fetchall('SELECT id,title,content,openid, pid,bid FROM ' . tablename('ewei_shop_sns_post') . (' WHERE id in( ' . $id . ' ) AND uniacid=') . $_W['uniacid']);
        ...
        foreach ($items as $item) {
            ...
            if ($home_recom) {
                plog('sns.posts.edit', '首页推荐' . $msg . ' ID: ' . $item['id'] . ' 标题: ' . $item['title'] . ' 内容: ' . $content);
                pdo_update('ewei_shop_sns_post', array('is_home_recom' => 1), array('id' => $item['id']));
                ...
                plog('sns.posts.edit', '取消首页推荐' . $msg . ' ID: ' . $item['id'] . ' 标题: ' . $item['title'] . ' 内容: ' . $content);
                pdo_update('ewei_shop_sns_post', array('is_home_recom' => 0), array('id' => $item['id']));
