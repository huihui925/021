...
class Partner_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar,dm.qqt_num from ' . tablename('ewei_shop_member') . ' dm ' . ' left join ' . tablename('ewei_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('ewei_shop_globonus_level') . ' l on l.id = dm.partnerlevel' . ' where dm.uniacid = ' . $_W['uniacid'] . (' and (dm.ispartner =1 or dm.ispartner=-1)  ' . $condition . ' ORDER BY dm.partnertime desc');
