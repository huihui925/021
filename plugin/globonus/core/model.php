	...
	class GlobonusModel extends PluginModel
	{
		public function getSet($uniacid = 0)
		{
			...
			$set['texts'] = array(
                ...
                'qqt_total' => empty($set['texts']['qqt_total']) ? '紫钻累计分红' : $set['texts']['qqt_total'],
				'qqt_lock' => empty($set['texts']['qqt_lock']) ? '紫钻待结算分红' : $set['texts']['qqt_lock'],
				'qqt_pay' => empty($set['texts']['qqt_lock']) ? '紫钻已结算分红' : $set['texts']['qqt_pay'],
			);
			return $set;
		}
...
感谢您的支持', 'color' => '#000000')
		...
		public function getBonusData($year = 0, $month = 0, $week = 0, $openid = '')
		{
            ...
            if (empty($set['qqtrate']) || $set['qqtrate'] <= 0) {
                $set['qqtrate'] = 100;
            }
			...
			if (!empty($bill) && empty($openid)) {
				return array(
					...
					'billid' => $bill['id'],
					'old' => true,
                    'qqt_nums'=>$bill['qqt_nums'],
                    'qqtrate' => $bill['qqtrate'],
                    'bonusrateordermoney'=>round($bill['bonusrateordermoney'], 2),
                    'qqtrateordermoney'=>round($bill['qqtrateordermoney'], 2)
				);
			}
			...
			$partners = pdo_fetchall('select m.id,m.openid,m.partnerlevel,l.bonus,m.qqt_num from ' . tablename('ewei_shop_member') . ' m ' . '  left join ' . tablename('ewei_shop_globonus_level') . ' l on l.id = m.partnerlevel ' . ('  where m.uniacid=:uniacid and  m.ispartner=1 and m.partnerstatus=1 ' . $pcondition), array(':uniacid' => $_W['uniacid']));
            ...
            //股东
            $partner_nums = pdo_fetch('select count(id) as partner_nums  from ' . tablename('ewei_shop_member') .' where uniacid=:uniacid and ispartner=1 and partnerstatus=1 LIMIT 1', array(':uniacid' => $_W['uniacid']))['partner_nums'];
            foreach ($orders as $o) {
				$ordermoney += $o['price'];
				...
				foreach ($partners as &$p) {
					...
					*/
					$p['bonusmoney'] += floatval($price * ($p['bonus'] / 100) * (1 / $partner_nums));
				}
            ...
            $qqtrateordermoney = 0;
            $bonusmoney2 = 0;
            $partners_qqt = pdo_fetchall('select m.id,m.openid,m.partnerlevel,l.bonus,m.qqt_num from ' . tablename('ewei_shop_member') . ' m ' . '  left join ' . tablename('ewei_shop_globonus_level') . ' l on l.id = m.partnerlevel ' . ('  where m.uniacid=:uniacid and  m.qqt_num > 0 ' . $pcondition), array(':uniacid' => $_W['uniacid']));
            $qqt_nums = pdo_fetch('select sum(qqt_num) as qqt_nums  from ' . tablename('ewei_shop_member') .' where uniacid=:uniacid and qqt_num>0 LIMIT 1', array(':uniacid' => $_W['uniacid']))['qqt_nums'];
            foreach ($orders as $o) {

                $qqtrateordermoney += $o['price'] * $set['qqtrate'] / 100;

                foreach ($partners_qqt as &$p) {
                    if (empty($set['selfbuy'])) {
                        if ($p['openid'] == $o['openid']) {
                            continue;
                        }
                    ...
                    $p['bonusmoney'] += floatval($o['price'] * ($set['qqtrate'] / 100) * ($p['qqt_num'] / $qqt_nums));
                }
            ...
            foreach ($partners_qqt as &$p) {
                ...
                $p['bonusmoney_send'] = $bonusmoney_send;
                $bonusmoney2 += $bonusmoney_send;
            }
            unset($p);

            $bonusrateordermoney = $bonusordermoney;
            $bonusordermoney += $qqtrateordermoney;
            $bonusmoney += $bonusmoney2;
			...
			if(!empty($openid)){
                $partners[0]['bonusmoney'] += $partners_qqt[0]['bonusmoney'];
                $partners[0]['bonusmoney_send'] += $partners_qqt[0]['bonusmoney_send'];
			}

            return array(
            	'orders' => $orders,
				'partners' => $partners,
				'partners_qqt'	=> $partners_qqt,
				...
				'old' => false,
				'qqt_nums'=>$qqt_nums,
				'qqtrate' => round($set['qqtrate'], 2),
				'bonusrateordermoney'=>round($bonusrateordermoney, 2),
				'qqtrateordermoney'=>round($qqtrateordermoney, 2)
			);
		}
