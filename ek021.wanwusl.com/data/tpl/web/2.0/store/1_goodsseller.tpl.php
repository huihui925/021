...
<div id="js-goods-seller" ng-controller="goodsSellerCtrl" ng-cloak>
	...
	<table class="table we7-table vertical-middle">
		...
		<tr>
			...
			<?php  if($type == STORE_TYPE_MODULE) { ?>
			<td>
				<?php  echo $module_types[$goods['type']]['title'];?>
			</td>
			<?php  } ?>
			<td>
		...
		<div class="modal-dialog  modal-dialog modal-lg we7-modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body material-content clearfix">
					<div class="material-nav" style="padding:0">
						<?php  if(is_array($module_types)) { foreach($module_types as $item) { ?>
						<a href="javascript:;" ng-click="tabChange('<?php  echo $item['sign'];?>_support', <?php  echo $item['type'];?>)"  ng-class="{true:'active',false:''}[moduleGoodsType=='<?php  echo $item['type'];?>']" ><?php  echo str_replace('应用', '', $item['title']);?></a>
						<?php  } } ?>
					</div>
